...
try {
    ...
    $packages = $repo->getPackagesWithOptionalDepDifferences();
} catch (Exception $e) {
    error_log("Error in report-optdep-differences.php: " . $e->getMessage(), 3, "/var/log/reporting.log");
    die("An internal error occurred. Please contact support.");
}

Layout::header('Optional Dependency Differences');
?>

<div class="container">
    <div class="card">
        <h2>🎁 Optional Dependency Differences</h2>
        <p style="margin-bottom: 15px; opacity: 0.8;">
            Packages with different optional dependencies between aarch64 and x86_64
            (optional features/functionality that can be enabled)
        </p>

        <?php if (empty($packages)): ?>
            <div class="alert alert-success">✓ All packages have the same optional dependencies across architectures!</div>
        <?php else: ?>
            <p style="margin-bottom: 15px; opacity: 0.7;">Found <?php echo count($packages); ?> package(s) with optional dependency differences</p>
            
            <div class="packages-table">
                <?php foreach ($packages as $pkg): ?>
                    <?php
                    $aarch64 = array_filter(array_map('trim', explode(',', $pkg['aarch64_optdeps'] ?? '')));
                    $x86_64 = array_filter(array_map('trim', explode(',', $pkg['x86_64_optdeps'] ?? '')));
