...
try {
    ...
    $packages = $repo->getPackagesWithMakedepDifferences();
} catch (Exception $e) {
    error_log("Error in report-makedep-differences.php: " . $e->getMessage(), 3, "/var/log/reporting.log");
    die("An internal error occurred. Please contact support.");
}

Layout::header('Makedepend Differences');
?>

<div class="container">
    <div class="card">
        <h2>🔨 Makedepend Differences</h2>
        <p style="margin-bottom: 15px; opacity: 0.8;">
            Packages with different build-time dependencies between aarch64 and x86_64
            (required only for building/compiling the package)
        </p>

        <?php if (empty($packages)): ?>
            <div class="alert alert-success">✓ All packages have the same makedepends across architectures!</div>
        <?php else: ?>
            <p style="margin-bottom: 15px; opacity: 0.7;">Found <?php echo count($packages); ?> package(s) with makedepend differences</p>
            
            <div class="packages-table">
                <?php foreach ($packages as $pkg): ?>
                    <?php
                    $aarch64 = array_filter(array_map('trim', explode(',', $pkg['aarch64_makedeps'] ?? '')));
                    $x86_64 = array_filter(array_map('trim', explode(',', $pkg['x86_64_makedeps'] ?? '')));
