...
try {
    ...
    $packages = $repo->getPackagesWithGroupDifferences();
} catch (Exception $e) {
    error_log("Error in report-group-differences.php: " . $e->getMessage(), 3, "/var/log/reporting.log");
    die("An internal error occurred. Please contact support.");
}

Layout::header('Group Membership Differences');
?>

<div class="container">
    <div class="card">
        <h2>👥 Group Membership Differences</h2>
        <p style="margin-bottom: 15px; opacity: 0.8;">
            Packages assigned to different groups between aarch64 and x86_64
            (e.g., base, base-devel, gnome, kde, etc.)
        </p>

        <?php if (empty($packages)): ?>
            <div class="alert alert-success">✓ All packages are in the same groups across architectures!</div>
        <?php else: ?>
            <p style="margin-bottom: 15px; opacity: 0.7;">Found <?php echo count($packages); ?> package(s) with group membership differences</p>
            
            <div class="packages-table">
                <?php foreach ($packages as $pkg): ?>
                    <?php
                    $aarch64 = array_filter(array_map('trim', explode(',', $pkg['aarch64_groups'] ?? '')));
                    $x86_64 = array_filter(array_map('trim', explode(',', $pkg['x86_64_groups'] ?? '')));
