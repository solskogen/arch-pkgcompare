...
Layout::header('aarch64 Outdated Packages');
?>

<div class="container">
    <div class="card">
        <h2>📦 aarch64 Outdated Packages (<?php echo count($outdated); ?>)</h2>
        <p style="margin-bottom: 15px; opacity: 0.8;">Packages where aarch64 version is behind x86_64</p>

        <div style="margin-bottom: 20px;">
            <a href="<?php echo Formatter::url('report-aarch64-ahead.php'); ?>" style="display: inline-block; padding: 10px 16px; background-color: var(--accent-color); color: white; border-radius: 4px; text-decoration: none; font-weight: 600;">
                ⚡ View aarch64 ahead (<?php echo count($ahead); ?>)
            </a>
        </div>

        <?php if (empty($outdated)): ?>
            <div class="alert alert-success">All aarch64 packages are up to date with x86_64! ✓</div>
        <?php else: ?>
            <table>
                <thead>
                    <tr>
