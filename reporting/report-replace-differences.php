...
try {
    ...
    $packages = $repo->getPackagesWithReplaceDifferences();
} catch (Exception $e) {
    error_log("Error in report-replace-differences.php: " . $e->getMessage(), 3, "/var/log/reporting.log");
    die("An internal error occurred. Please contact support.");
}

Layout::header('Replace Differences');
?>

<div class="container">
    <div class="card">
        <h2>🔄 Package Replace Differences</h2>
        <p style="margin-bottom: 15px; opacity: 0.8;">
            Packages with different replace definitions between aarch64 and x86_64
            (packages that can replace other packages during upgrades)
        </p>

        <?php if (empty($packages)): ?>
            <div class="alert alert-success">✓ All packages have the same replaces across architectures!</div>
        <?php else: ?>
            <p style="margin-bottom: 15px; opacity: 0.7;">Found <?php echo count($packages); ?> package(s) with replace differences</p>
            
            <div class="packages-table">
                <?php foreach ($packages as $pkg): ?>
                    <?php
                    $aarch64 = array_filter(array_map('trim', explode(',', $pkg['aarch64_replaces'] ?? '')));
                    $x86_64 = array_filter(array_map('trim', explode(',', $pkg['x86_64_replaces'] ?? '')));
